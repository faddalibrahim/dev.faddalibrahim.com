...
images: ['/static/images/nested-comments/cover-img.svg']
---
...
Again, from Part 1 of this post, we deduced that we needed to know **comment's id** and its **parent's id**. If it has no parent id, then its a top level comment, other wise its a nested comment reply.
