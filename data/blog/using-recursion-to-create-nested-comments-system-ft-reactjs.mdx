...
Here is a [live link](https://react-nested-comments-system.netlify.app) to the final version of the app. This post doesn't highlight anything about styling, layout and other aesthetics you might see in the final version. The full code is hosted on this [github link](https://github.com/faddalibrahim/nested-comments-system-react) for your perusal.

With that being said, we are good to go. Find below the roadmap!

## Goals?

My goal for this post in not to spoon feed you with code that you can plug into your app (chatGPT can help you with that), but to breakdown the mechanics of the behavior of a nested comments system and make you understand what is happening on a deeper level. Ultimately, you should be able to recreate the logic and customize it to feed your needs. Also, its a future time-saver for me because I have a really terrible memory and these technical articles I write come in handy whenever I need them.
...
The recursive flow of this function mimics the behavior of a nested comments system -- We render a parent comment, then proceed to render all the sub comments in it by recursively, till there are no more sub-comments.
...
1. The main components of a comment we will focus on are `text` and `children/sub comments`. The sub comments/children are what we linguistically call a reply. But lets call them children for the sake of technicality.
...
So, there is literally no ROOT. But we can create an imaginary/fake ROOT comment. This imaginary/fake root will have no `text`, but its children will be these `Top-Level` Comments. This makes it possible to pass a ROOT to initiate the recursive chain.
...
To add a comment to the comment tree, we need to know exactly where to attach it. A comment can either be a top-level comment or a sub-comment (reply).

For sub-comments, we need to know the **id** of the parent comment they belong to.  
Top level comments dont have any parents, so we can add them directly to the children of the **imaginary/fake** root.
...
function addCommentWithDFS(
  ...
  for (let i = 0; i < commentRoot.children.length; i++) {
    const currentComment = commentRoot.children[i];
...
That is it! All we need to do now is implement this same logic using a frontend framework of our choice. Man, I love ReactJS. Let's use that
...
Let's extend the previous **Comment** interface to include **id** and **user**.
...
By looping through and passing each comment's data to `CommentBlock.tsx`, it starts the recursive chain that results in the rendering of sub comments for each parent comment.
...
Finally, **CommentSection.tsx** encloses everything. It is also where comment state is defined and pass down as props to the sub components.
...
Since we are using ReactJS, lets think react-wise. To perform any modifications to the commen tree, you guessed it -- we will use **setAllComments** set state action.

Its important to note that adding a comment to the tree will be in 2 ways. Either as a **top-level comment** or as a sub-comment/reply to another comment. Hence, the way in which we use **setAllComments** to modify the state will be different.
...
For this, we need to know exactly where to insert the comment. What do we need to know? The **id** of the parent comment under which we are inserting into.
...
Applying the same idea as adding a comment
...
I believe the code samples and illustrations did well in explaining the logic behind the comments system. Armed with this knowledge, I beleive you can piece it together to customize yours further, although you are welcome to take a cue from mine. Its more important that you understand and be able to redo from scratch!

In the final version of my code on github. Its a bit more complex that the simple demonstration here. I had to use react context to prevent prop drilling.

In trying to modularize the code a bit furhter, I ended up with prop drilling and passing

Interesting things you can add

1. connect to a nosql database (firebase/supabase)
2. web sockets to realtime comment updates from different users
