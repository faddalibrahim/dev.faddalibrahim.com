...
Moreover, there are problems that are just too difficult or even impossible to solve without recursion. Think of a social media comment section like reddit or twitter, where you can reply to a comment, and then someone else can reply to your reply, and so on. This is a classic example of a **nested comments system**. The depth of the nesting is not known beforehand and can be deeper on many lower levels. This is a perfect use case for recursion.
...
1. Build out the foundational logic for the behavior of the nested comments system using textbook/classroom knowledge
...
1. The main components of a comment we will focus on are `text` and `children/sub comments`. The **sub comments/children** are what we linguistically call **replies**. But lets call them **children** for the sake of technicality.
...
Just becuase you can solve it recursively doesn't mean you should! If there is an iterative solution that is more efficient, then you should use that instead.
...
This has been a fun ride for me. From diagramming, editting, debugging, coding, testing and to the final version of this post. There is many more ideas currently in development and if you loved this, you should consider subsribe down below. Your comments and feedback are welcome as well.
