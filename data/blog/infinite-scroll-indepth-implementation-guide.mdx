...
On a basic technical level, new content is fetched from the server when the user reaches the bottom of the currently loaded content. The newly fetched content is then dynamically added to the page, creating the illusion of an endless stream of information.

This post focuses on the indepth technical logic and implementation (using ReactJS) behind this technique.

Find here the complete [code]() and [demo]()

<TOCInline toc={props.toc} exclude="Introduction" />

## How do we know we need to fetch more content?

The most important question to ask now is **How do we know we are at the bottom of the page and therefore need to fetch in more content?** There are 2 ways to do this : **Web Geometry** and **Intersection Observer**.
